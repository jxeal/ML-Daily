      ...
      <div className="flex items-center justify-between max-w-5xl mx-auto">
        <Link href="/">
          <div className="flex items-center gap-1.5 sm:gap-2 cursor-pointer group">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-tr from-primary to-emerald-400 flex items-center justify-center shadow-lg shadow-primary/20 group-hover:scale-105 transition-transform shrink-0">
              <Brain className="w-5 h-5 sm:w-6 sm:h-6 text-primary-foreground" />
            </div>
            <h1 className="text-lg sm:text-xl font-display font-bold tracking-tight group-hover:text-primary transition-colors whitespace-nowrap">ML Daily</h1>
          </div>
        </Link>

        <div className="flex items-center gap-1.5 sm:gap-3">
          <ThemeToggle />
          <div className="flex items-center gap-1.5 bg-secondary/80 backdrop-blur-md px-2 sm:px-3 py-1.5 rounded-full border border-white/5">
            <Flame 
              className={`w-4 h-4 ${streak > 0 ? 'text-warning fill-warning' : 'text-muted-foreground'}`} 
            />
            <span className="font-bold text-sm">
              {streak} <span className="hidden min-[400px]:inline text-muted-foreground font-medium">Day{streak !== 1 ? 's' : ''}</span>
            </span>
          </div>
