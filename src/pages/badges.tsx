...
export default function Badges({ params }: { params?: { username?: string } }) {
  ...
  const { data: publicStats, isLoading: publicStatsLoading } = useUserProfile(
    params?.username || ""
  );
  ...
  const isLoading =
    authLoading || statsLoading || publicStatsLoading || allBadgesLoading;
  ...
  if (isPublicView) {
    ...
    stats = user
      ? {
          badges: Array.isArray(supabaseStats?.badges)
            ? supabaseStats.badges
            : [],
        }
      : {
  ...
  const getBadgeIcon = (iconName: string, className: string = "w-8 h-8") => {
    switch (iconName) {
      ...
      case "Trophy":
        return <Trophy className={`${className} text-yellow-600`} />;
      case "Target":
        return <Target className={`${className} text-red-500`} />;
      default:
        return <Award className={`${className} text-accent`} />;
    }
  };

  // Resolve badge details
  const resolvedBadges = stats.badges
    .slice()
    .reverse()
    .map((b: any) => {
      ...
      const dbBadge = allBadges?.find(
        (dbB) => dbB.id === badgeIdOrName || dbB.name === badgeIdOrName
      );
  ...
  const isCatalogEmpty =
    !allBadgesLoading && (!allBadges || allBadges.length === 0);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {displayUser.full_name}'s Collection
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {isCatalogEmpty ? (
            <div className="p-12 text-center border border-dashed border-border rounded-[2rem] text-muted-foreground bg-card/30">
              ...
              <p>
                The achievement catalog is currently empty. Check back later!
              </p>
            </div>
            ...
            resolvedBadges.map((badge, idx: number) => (
              <motion.div
                ...
                className="group relative overflow-hidden bg-card border border-border rounded-[2rem] flex flex-col sm:flex-row items-stretch shadow-sm hover:shadow-md transition-all duration-300"
              >
                {/* Left Side - Badge Icon (1/3) */}
                <div className="sm:w-1/3 p-8 flex flex-col items-center justify-center bg-gradient-to-br from-secondary/50 to-secondary/10 border-b sm:border-b-0 sm:border-r border-border relative overflow-hidden">
                  <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(0,0,0,0.02)_0%,transparent_70%)] dark:bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.05)_0%,transparent_70%)]" />
                ...
                <div className="sm:w-2/3 p-8 flex flex-col justify-center">
                  ...
                  <div className="mt-auto pt-6 border-t border-border/50 relative">
                    <span className="absolute -top-3 left-4 bg-card px-2 text-4xl text-primary/20 font-serif leading-none">
                      "
                    </span>
