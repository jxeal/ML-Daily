      ...
      <div className="animate-in slide-in-from-bottom-4 duration-500 pb-20">
        ...
        <div className="relative pt-6 px-4 md:px-8 pb-12 border-b border-white/5 bg-gradient-to-b from-card to-background">
          ...
          <div className="flex items-center gap-3 mb-4">
            <span className="text-4xl bg-secondary/50 p-3 rounded-2xl">{lesson.icon}</span>
            <div>
          ...
          <div className="prose prose-invert prose-emerald max-w-none">
            ...
            */}
            {lesson.content.split('\n\n').map((paragraph, idx) => (
              <p key={idx} className="text-[1.1rem] leading-relaxed text-foreground/90 mb-6">
                {paragraph}
              </p>
            ))}
          </div>
            ...
            <div className="my-12">
              ...
              <div className="space-y-4">
                {lesson.examples.map((ex, idx) => (
                  <div key={idx} className="bg-card p-6 rounded-2xl border-l-4 border-accent shadow-sm">
                    <p className="font-mono text-sm leading-relaxed text-muted-foreground">{ex}</p>
                  </div>
                ))}
              </div>
            </div>
